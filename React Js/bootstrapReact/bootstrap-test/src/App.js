...
//       {/* {navshow && <div className={`${navshow ? 'overLay' : '' } onClick={() => setNavshow(false)`}></div>} */}
//       {/* {navshow && <div className="overLay" onClick={() => setNavshow(false)}></div>} */}
//       {/* <div className={`${navshow ? 'overLay' : ''}`}></div> */}


//       <button className='navBtn' onClick={() => setNavshow(!navshow)}>{navshow ? <span>&times;</span> : <span>&#9776;</span>}</button>

//       {/* <div className={!navshow ? 'navSlide' : 'showNav'}>   */}
...
//     // this just removes written in list from item not completely remove from list beacuse the map function can only remove at condition which indexNumber != index it return same length of array not new length of array

//     // let afterDeleteFinalTodoList = todoList.map((item, index) => {
